...
- Валидация заголовков `Origin` и `Referer` по строгому белому списку разрешенных доменов (`server.cors_origins`).
- Обязательное требование заголовка `X-Requested-With` для асинхронных API вызовов.

### 3.4 Rate Limiting и аудит
- Алгоритм скользящего окна (Sliding Window) с поддержкой распределенных хранилищ (Redis, Postgres, SQLite).
