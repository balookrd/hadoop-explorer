<script lang="ts">
  ...
  function actionBadge(action: string): string {
    switch (action) {
      case 'created': return 'bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300';
      case 'modified': return 'bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300';
      case 'deleted': return 'bg-red-100 dark:bg-red-950/60 text-red-700 dark:text-red-300';
...
{#if isOpen}
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 dark:bg-slate-950/80 backdrop-blur-sm p-4">
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col border border-slate-200 dark:border-slate-800 overflow-hidden">
      <!-- Header -->
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800">
        <div>
          <h2 class="text-sm font-bold text-slate-900 dark:text-slate-100">Changes Review (Live → Draft)</h2>
          <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{totalChangesCount} change(s)</p>
        </div>
      ...
      <!-- Content -->
      <div class="flex-1 overflow-auto divide-y divide-slate-100 dark:divide-slate-800">
        <!-- Queue Mappings Diff Card -->
        {#if hasMappingsDiff && queueMappingsDiff}
          <div class="p-4 bg-indigo-50/50 dark:bg-indigo-950/30 border-b border-indigo-100 dark:border-indigo-900/50">
            <div class="flex items-center gap-2 mb-2">
              <ArrowRightLeft class="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
              <span class="text-xs font-bold text-slate-900 dark:text-slate-100">Queue Mappings & Overrides</span>
              <span class="text-[10px] px-2 py-0.5 rounded-full font-bold bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300">modified</span>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
              <div class="bg-white dark:bg-slate-950 rounded-lg p-2.5 border border-slate-200 dark:border-slate-800">
                <div class="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 mb-1">Live Configuration</div>
                <div class="font-mono text-[11px] text-slate-700 dark:text-slate-300 break-all">
                  {queueMappingsDiff.live_mappings || '<пусто>'}
                </div>
                <div class="mt-1.5 text-[11px] text-slate-500 dark:text-slate-400">
                  Override enabled: <span class="font-semibold text-slate-700 dark:text-slate-300">{queueMappingsDiff.live_override ? 'true' : 'false'}</span>
                </div>
              </div>

              <div class="bg-indigo-50/80 dark:bg-indigo-950/60 rounded-lg p-2.5 border border-indigo-200 dark:border-indigo-800">
                <div class="text-[10px] uppercase font-bold text-indigo-600 dark:text-indigo-400 mb-1">Draft Configuration</div>
                <div class="font-mono text-[11px] text-indigo-950 dark:text-indigo-200 font-semibold break-all">
                  {queueMappingsDiff.draft_mappings || '<пусто>'}
                </div>
                <div class="mt-1.5 text-[11px] text-indigo-900 dark:text-indigo-300">
        ...
        {#if changedDiffs.length > 0}
          <table class="w-full text-xs">
            <thead class="bg-slate-50 dark:bg-slate-950 sticky top-0 border-b border-slate-200 dark:border-slate-800">
                ...
                <tr class="border-b border-slate-100 dark:border-slate-800/60 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  <td class="px-4 py-2 font-mono text-slate-800 dark:text-slate-200">
                    ...
                    {#if d.draft_user_limit_factor != null && d.live_user_limit_factor !== d.draft_user_limit_factor}
                      <div class="text-[10px] text-sky-700 dark:text-sky-400 font-sans mt-0.5">
                    ...
                    {#if d.draft_ordering_policy && d.live_ordering_policy !== d.draft_ordering_policy}
                      <div class="text-[10px] text-purple-700 dark:text-purple-400 font-sans mt-0.5">
                    ...
                    {#if d.draft_max_applications != null && d.live_max_applications !== d.draft_max_applications}
                      <div class="text-[10px] text-indigo-700 dark:text-indigo-400 font-sans mt-0.5">
                  ...
                  <td class="text-right px-2 py-2 font-mono text-slate-500 dark:text-slate-400">
                    {d.live_capacity != null ? `${d.live_capacity.toFixed(1)}%` : '—'}
                  </td>
                  <td class="text-center px-2 py-2 text-slate-400 dark:text-slate-500">→</td>
                  <td class="text-right px-2 py-2 font-mono font-bold {
                    d.delta_capacity != null && d.delta_capacity > 0 ? 'text-emerald-600 dark:text-emerald-400' :
                    d.delta_capacity != null && d.delta_capacity < 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-800 dark:text-slate-200'
                  }">
                  ...
                  <td class="text-center px-2 py-2 text-slate-400 dark:text-slate-500">→</td>
                  <td class="text-right px-2 py-2 font-mono font-bold {
                    d.delta_max_capacity != null && d.delta_max_capacity > 0 ? 'text-emerald-600 dark:text-emerald-400' :
                    d.delta_max_capacity != null && d.delta_max_capacity < 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-800 dark:text-slate-200'
                  }">
      ...
      <!-- Footer -->
      <div class="flex items-center justify-between px-6 py-3 border-t border-slate-200 dark:border-slate-800">
        <button onclick={() => isOpen = false}
          class="px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition cursor-pointer">
          Закрыть
        </button>
        <div class="flex items-center gap-2">
          {#if onSubmitCr}
            <button
              ...
              class="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 text-xs font-semibold shadow-xs transition cursor-pointer"
