...
<div class="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50 select-none">
  <div class="w-full max-w-md bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl shadow-2xl p-6 sm:p-8 flex flex-col gap-5 text-slate-800">
    ...
    <!-- Форма LDAPS -->
    <form onsubmit={handleLdapLogin} class="flex flex-col gap-3.5">
      <div>
        ...
        <div class="relative">
          ...
          <input
            type="text"
            bind:value={username}
            placeholder="например, analyst_user"
    ...
    <div class="pt-3 border-t border-slate-200">
      ...
      <div class="flex flex-col gap-1.5 text-xs">
        ...
        >
          <div>
            <div class="font-bold text-slate-800">Анна Аналитикова (analyst_user)</div>
            <div class="text-[10px] text-slate-500">Группы: bi-analysts (Trino + Hive Analytics)</div>
          </div>
        ...
        >
          <div>
            <div class="font-bold text-slate-800">Иван Датаинженеров (de_user)</div>
            <div class="text-[10px] text-slate-500">Группы: data-engineers (доступ к DE Cluster)</div>
          </div>
