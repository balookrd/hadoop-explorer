...
async def login(login_req: LoginRequest, request: Request, response: Response):
    ...
    mode = (
        ...
        or ("mock" if not settings.ldap.enabled else "ldaps_only")
    )
    user = None
    if mode == "mock":
    ...
    return LoginResponse(success=True, user=user, message="Авторизация успешна")
...
async def kerberos_sso(request: Request, response: Response):
    ...
    if not user:
        user = UserInfo(username=username, display_name=username, groups=[])
...
async def logout(request: Request, response: Response):
    ...
    if token:
        ...
        storage_service.revoke_token(token_or_jti=token, username=username, expires_at=payload.exp if payload else None)
        if payload and payload.jti:
            storage_service.revoke_token(token_or_jti=payload.jti, username=username, expires_at=payload.exp)
