...
1. **Spark Explorer UI & Backend** (порт `8004`):
   ...
   - Единая UI-компоновка: управление Livy-сессией вынесено в шапку (`SparkSessionWidget`), верхняя панель вкладок скриптов/ноутбуков и локальный тулбар сессии (`SessionBar`).
   - Отображение эффективной роли пользователя (`ADM`, `RW`, `RO`) в профиле.
   - Управление сессиями в реальном времени, привязка к YARN Application ID и автоматическая очистка сессий при выходе пользователя.
