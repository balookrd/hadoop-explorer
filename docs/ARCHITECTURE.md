...
- **Предпросмотр данных**: потоковое чтение и конвертация форматов Apache Parquet и ORC в структурированный JSON прямо в памяти сервера без выгрузки на диск хоста. Для крупномасштабных Parquet-файлов, превышающих лимит полного чтения, реализовано извлечение схемы колонок и метаданных через чтение футера файла по диапазону (Range Reading).
