...
10. [Процесс согласования изменений (Approval Workflow)](#10-процесс-согласования-изменений-approval-workflow)
...
graph TD
    ...
    G -->|Согласование по принципу Four-Eyes| H
    H -->|yarn rmadmin -refreshQueues| A
```
...
- 📄 **Бесшовный экспорт в XML**: формирование валидного `capacity-scheduler.xml` с готовыми CLI-инструкциями для применения без перезагрузки кластера (`yarn rmadmin -refreshQueues`).
...
│      - Elasticity Ratio (коэффициент эластичности)                                           │
...
Администраторы могут напрямую сформировать финальный файл конфигурации, нажав **«Сгенерировать XML»**:

<div align="center">
  <img src="images/yarn/10_xml_export_modal.png" alt="Модальное окно генерации и экспорта XML" width="850" />
  <p><em>Рисунок 10: Модальное окно генерации и экспорта capacity-scheduler.xml</em></p>
</div>

### Экспорт и инструкции:
1. **Выбор формата**: кнопка переключения позволяет сгенерировать конфигурацию как в классическом процентном формате (`%`), так и в абсолютном ресурсном формате (`Absolute Resource Allocation`).
2. **Экспорт**:
   - **«Копировать XML»**: копирует весь текст конфигурации в буфер обмена.
   - **«Скачать XML»**: скачивает готовый файл с именем `capacity-scheduler-<cluster_id>-<timestamp>.xml`.

### Применение на кластере (Zero Downtime):
