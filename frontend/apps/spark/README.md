...
├── components/
│   ├── SparkSessionWidget.svelte # Виджет сессии Livy в панели вкладок (статус, кластер, рестарт, кнопка Configure)
│   ├── SessionBar.svelte     # Локальный тулбар активной вкладки: запуск Run, Cancel, выбор языка (PySpark/Scala/SQL), таймер, бейдж строк
...
- **SparkSessionWidget справа от вкладок скриптов**: состояние интерактивной Livy-сессии гармонично встроено в строку вкладок рядом со скриптами (по аналогии с Databricks и JupyterLab: статус `idle`/`busy`/`dead`, целевой кластер, кнопка перезапуска/остановки и вызов модального окна настроек).
...
- **Локальный тулбар вкладки (`SessionBar`)**: компактно расположен под вкладками и включает селектор языка активного скрипта (PySpark, Scala, SQL), кнопки Run и Cancel, таймер выполнения и унифицированный зелёный бейдж количества строк.
